...
- **Deterministic rule engine, not AI.** "This address → this AHJ → adopted NEC 2020 + amendment set → this feeder/panel/circuit needs X." Every flag defensible in front of the inspector.
- **Verified-vs-unverified trust model.** Every rule is timestamped, sourced to the adopting ordinance, and marked `verified-by-outcome` vs `unverified`. Never present an unverified rule as certainty. That honesty is what makes inspectors and electricians willing to feed it — which is what makes it grow — which is what makes it the standard. Under-claim and it compounds; over-claim once and it dies.
- **Seed one jurisdiction, then spread.** You do not need 20,000 jurisdictions on day one. Anchor on **AJ's home turf (upstate NY — confirm exact AHJ/county)** so every rule can be validated firsthand, starting with the single highest-value, most-often-wrong flag: *which NEC edition this AHJ has adopted*. Then layer amendments city-by-city as logged inspection outcomes harden the dataset.
